...
export default (props) => {
    ...
    return (
        <div {...props}>
            <div className="mb-4">
                <span>{props.advanced ? "Parametere" : "Tiltak"} {vehicles[props.vehicle]}</span>
                <hr className="mt-1 mb-4" />
                {!props.advanced && props.vehicle &&
                    <>
                        <div className="text-center">
                            {policiesRef.current.map((policy, index) => {
                                if (policy.showButton.includes(props.vehicle)) {
                                    return <Form.Check
            ...
            {props.advanced &&
                <Row>
                    {percentages.map((percentage, index) => {
                        if (percentage.showButton.includes(props.vehicle)) {
                            return <Col
                                ...
                                <Form.Group>
                                    ...
                                    <InputGroup>
                                        <Form.Control
                                            ...
                                            isInvalid={props.errors[`${props.vehicle}${percentage.id}`]}
                                            ref={props.register({
                                                required: true,
                                                min: percentage.min,
                                                max: percentage.max,
